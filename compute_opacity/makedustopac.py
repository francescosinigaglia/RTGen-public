# Original code is part of the RADMC-3D radiative transfer tool.
# http://www.ita.uni-heidelberg.de/~dullemond/software/radmc-3d/
# Original code is developed by C. P. Dullemond.
#
# Modified by Laszlo Szucs, 2018
#
# Change list:
#  - optional mode for computing opacity of a broad grain size
#    distribution. Arbitrary grain size distribution with user
#    defined weighting. ( compute_opac_mie() )
#
...
def compute_opac_mie(optconst_file,matdens,agraincm,lamcm,wgt=None,
    ...
    #
    # TODO Document the new feature / parameters (agraincm can be array, wgt input)
    # TODO Make sure that [agrain] and logawidth is note used together
    # TODO Add option to write out opacity for all grain sizes
    if (type(agraincm) in [list, np.ndarray]) and (logawidth is None):
        agr = np.array(agraincm)
        wgt = np.array(wgt)/np.array(wgt).sum()
    elif logwidth:
...
def write_radmc3d_kappa_file(package,name,filename=None):
    """
    The RADMC-3D radiative transfer package
      http://www.ita.uni-heidelberg.de/~dullemond/software/radmc-3d/
    can perform dust continuum radiative transfer for diagnostic purposes.
