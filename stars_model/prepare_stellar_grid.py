...
mass_star_sph = pars.input_dir + pars.mass_star_sph_filename
...
grid = np.zeros((ngridx,ngridy,ngridz,nmassbins))

# Read mass SPH
mass_sph = np.fromfile(mass_star_sph, dtype=np.float64)
mass_sph = np.reshape(mass_sph, (ngridx,ngridy,ngridz), order='F')

for ii in range(nmassbins):
    grid[:,:,:,ii] = mass_sph
...
# Weights
ww = masslin * imf / np.sum(masslin * imf)

# Write the stellar grid to file
grid = grid * ww / (4*np.pi)
